...
async def send_pricelist(bot):
    ...
    for p in products:
        ...
        try:
            ...
            logger.error(f"خطا در ارسال محصول: {e}")
            await bot.send_message(chat_id=CHANNEL_ID, text=caption)
...
async def adm_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    for i, p in enumerate(products):
        ...
        keyboard.append([InlineKeyboardButton(f"{status}{photo} {i+1}. {p['name'][:30]}", callback_data=f"adm_p_{i}")])
...
async def adm_product(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("📸 تغییر/افزودن عکس", callback_data=f"adm_photo_{idx}")],
        ...
        [InlineKeyboardButton("🗑 حذف", callback_data=f"adm_del_{idx}")],
...
async def adm_start_price(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    context.user_data["edit_idx"] = idx
    products = load_products()
    await query.edit_message_text(f"💰 قیمت فعلی: {products[idx]['price']}\n\nقیمت جدید را بنویسید:")
...
async def adm_start_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    products = load_products()
    await query.edit_message_text(f"📸 عکس جدید برای:\n{products[idx]['name']}\n\nعکس را بفرستید:")
    return WAIT_PHOTO
...
async def adm_save_photo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    products[idx]["photo_id"] = photo_id
    save_products(products)
